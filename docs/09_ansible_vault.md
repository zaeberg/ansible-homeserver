...
В текущем состоянии системы Vault особенно полезен для следующих задач:

- задать фиксированный пароль для `Traefik` dashboard вместо автоматически сгенерированного;
- ввести управляемый `sudo`-пароль пользователя `admin` через `admin_password_hash`.
- хранить чувствительные переменные `Homepage` в `homepage_env` вместо открытого `.env` в роли.

После появления `admin_password_hash` и проверки доступа под `admin` через `sudo` можно переводить:

- `admin_nopasswd_sudo: false`

После этого playbook запускается либо с `--ask-become-pass`, либо с vaulted `ansible_become_password`.
